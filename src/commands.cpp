...
void Server::part(std::vector<std::string> reqVec, Client& client)
{
	if (reqVec.size() > 1)
	{
		for (std::vector<Channel*>::iterator itClientChan = client.getJoinedChannels().begin(); itClientChan != client.getJoinedChannels().end(); ++itClientChan)	//iterating through Clients joined channel list
		{
			if ((*itClientChan)->getName() == reqVec[1])
			{
				std::string response = ":" + client.getNickname() + "!~" + client.getUsername() + "@127.0.0.1 PART " + reqVec[1] + "\r\n";
				send(client.getSock(), response.c_str(), response.size(), 0);
				for (std::vector<Client*>::iterator itClient = (*itClientChan)->getClients().begin(); itClient != (*itClientChan)->getClients().end(); ++itClient)	//iterating through joined channels clients, finding "itself" and erase field in vec
				{
					if ((*itClient)->getNickname() == client.getNickname())
					{
						(*itClientChan)->getClients().erase(itClient);
						break ;
					}
				}
				this->sendMsgToAll(client, response);

				size_t numClients = (*itClientChan)->getClients().size(); //store size to catch empty channel (for channel deletion)
				
				std::cout << GRAY << client.getNickname() << " left channel: " << RESET << (*itClientChan)->getName() << std::endl;
				
				itClientChan = client.getJoinedChannels().erase(itClientChan); //removing channel from clients joined channels
	...
	if (reqVec.size() > 1)
	{
		...
		for (std::vector<Channel>::iterator itChannel = this->_channels.begin(); itChannel != this->_channels.end(); ++itChannel)
		{
			if ((*itChannel).getName() == reqVec[1])
			{
				...
				client.getJoinedChannels().push_back(&(*itChannel));
